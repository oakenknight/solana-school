...
declare_id!("85aPRtCLLVwkf9hNi1xMZ4Q3ZLuQXZaauXNx3QuEbstc");
...
pub mod reinit_attack {
    ...
    pub fn drain_lamports(ctx: Context<DrainLamports>) -> Result<()> {
        ...
        msg!("Draining {} lamports", lamports);
        msg!(
            "Signer has {} lamports before",
            ctx.accounts.signer.to_account_info().lamports()
        );
    ...
    pub fn give_to_system_program(ctx: Context<GiveToSystemProgram>) -> Result<()> {
        ...
        let lamports = ctx.accounts.signer.to_account_info().lamports();
        msg!("Signer has {} lamports", lamports);
