...
import { Day36 } from "../target/types/day_36";
...
describe("day_36", () => {
  ...
  const program = anchor.workspace.Day36 as Program<Day36>;
...
function toDisplayAmount(amount: number): number {
  return amount / 1e9; // Convert from lamports/smallest unit to display units (divide by 10^9)
}
function lamportsToSol(lamports: anchor.BN): number {
  return Number(lamports) / 1e9; // Convert from lamports to SOL (divide by 10^9)
}
function toRawTokenAmount(displayAmount: number): number {
  return displayAmount * 1e9; // Convert from display units to raw token amount (multiply by 10^9)
}
